...
-- 基地の座標 (画像より)
local BASE_COORDINATES = CFrame.new(766.0, 2022.1, -1149.5)
-- ゴールの座標
...
container.CanvasSize = UDim2.new(0, 0, 0, 550) -- ボタンが増えたので拡張
...
local speedBtn = createButton("speed", "speed", function()
...
createButton("baseTP", "baseTP", function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        LocalPlayer.Character.HumanoidRootPart.CFrame = BASE_COORDINATES
    end
end)
...
local function applyESP(player)
    ...
    local function setupChar(character)
        ...
        local conn; conn = RunService.RenderStepped:Connect(function()
            ...
            local s = getgenv().ShiunSettings
            highlight.Enabled = (s.nameEnabled or s.healthEnabled)
            tag.Enabled = (s.nameEnabled or s.healthEnabled)
            nl.Visible = s.nameEnabled
            hb.Visible = s.healthEnabled
